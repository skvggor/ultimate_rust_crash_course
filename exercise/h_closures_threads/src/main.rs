...
fn main() {
    ...
    let (challenge_tx, challenge_rx) = channel::unbounded();
    let challenge_rx1 = challenge_rx.clone();
    let challenge_rx2 = challenge_rx.clone();

    challenge_tx.send("Main Thread Challenge: 1").unwrap();
    challenge_tx.send("Main Thread Challenge: 2").unwrap();

    let handle_challenge1 = thread::spawn(move || {
        if let Ok(msg) = challenge_rx1.recv() {
            println!("Thread 1 received: {}", msg);
        }
    });

    let handle_challenge2 = thread::spawn(move || {
        if let Ok(msg) = challenge_rx2.recv() {
            println!("Thread 2 received: {}", msg);
        }
    });

    // for i in 0..10 {
    //     let msg = format!("Thread Challenge: {}", i);
    //     challenge_tx.send(move || msg).unwrap();
    // }

    drop(challenge_tx);

    handle_challenge1.join().unwrap();
    handle_challenge2.join().unwrap();
